...
static uint8_t *load_first_image(const char *path, igb *file, igb_image *image,
                                 int *rgba_len)
{
...
static image_diff compare(const uint8_t *actual, const uint8_t *oracle,
                          size_t pixels)
{
...
static void verify_pair(const char *ps2_path, const char *x360_path,
                        int expected_format, size_t expected_exact,
                        uint64_t expected_sad)
{
    igb ps2_file;
    igb x360_file;
    igb_image ps2_image;
    ...
    uint8_t *x360_rgba =
        load_first_image(x360_path, &x360_file, &x360_image, &x360_len);
    ...
    printf("real format=%d exact=%zu/%zu rgba_sad=%llu/%zu\n",
           expected_format, diff.exact, diff.pixels,
           (unsigned long long)diff.sad, diff.pixels * 4);
...
int main(int argc, char **argv)
{
    ...
    int present = path_exists(ps2_font) + path_exists(x360_font) +
                  path_exists(ps2_default) + path_exists(x360_default);
    if (present == 0) {
    ...
    verify_pair(ps2_font, x360_font, ALCHEMY_IGB_PFMT_CLUT_INDEX8, 40656,
                font_sad);
    verify_pair(ps2_default, x360_default, ALCHEMY_IGB_PFMT_RGBA5551, 0,
                5760);
