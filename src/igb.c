...
static int32_t rd_i32(const uint8_t *p, int le) { return (int32_t)rd_u32(p, le); }
...
static int is_mem_dir(const igb_ctx *c, uint32_t type_idx) {
  return type_idx < c->n_meta_objs && c->meta_objs[type_idx].name &&
...
static int deserialize_field(const igb_ctx *c, uint32_t type_idx, uint32_t slot, uint32_t size,
                             const uint8_t *data, size_t avail, igb_fieldval *out) {
  ...
  if (strcmp(out->short_name, "ObjectRef") == 0 || strcmp(out->short_name, "MemoryRef") == 0) {
  ...
  if (strcmp(out->short_name, "CharArray") == 0 ||
      ...
      strcmp(out->short_name, "FloatArray") == 0 || strcmp(out->short_name, "IntArray") == 0 ||
      strcmp(out->short_name, "UnsignedIntArray") == 0 ||
      strcmp(out->short_name, "ShortArray") == 0 ||
      strcmp(out->short_name, "UnsignedShortArray") == 0 || strcmp(out->short_name, "Vec2f") == 0 ||
      strcmp(out->short_name, "Vec3f") == 0 || strcmp(out->short_name, "Vec4f") == 0 ||
      strcmp(out->short_name, "Quaternionf") == 0 || strcmp(out->short_name, "Matrix44f") == 0) {
...
int igb_open(igb *f, const char *path) {
  ...
  if (mf_count) {
    ...
    for (uint32_t i = 0; i < mf_count; ++i) {
      ...
      if (name) {
        ...
        memcpy(c.meta_fields[i].short_name, "Unknown", sizeof("Unknown"));
      }
      dyn_off += name_len;
    }
  }
  ...
  for (uint32_t i = 0; i < c.n_meta_objs; ++i) {
    ...
    f->meta[i].fields =
        calloc(c.meta_objs[i].n_fields ? c.meta_objs[i].n_fields : 1, sizeof(igb_meta_field_desc));
...
uint8_t *igb_image_to_rgba(const igb_image *img, int *out_len) {
  *out_len = 0;
  ...
  int block_size = (img->pixel_format == ALCHEMY_IGB_PFMT_RGB_DXT1 ||
                    img->pixel_format == ALCHEMY_IGB_PFMT_RGBA_DXT1)
                       ? 8
                       : 16;
