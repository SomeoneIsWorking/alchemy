...
static void dump_object(const igb *f, int idx) {
  ...
  printf("object[%d] type=%s is_mem=%d mem_size=%zu n_fields=%d\n", idx,
         o->type_name ? o->type_name : "?", o->is_mem, o->mem_size, o->n_fields);
  for (int i = 0; i < o->n_fields; ++i) {
    const igb_fieldval *fv = &o->fields[i];
    if (fv->blob && fv->blob_len) {
      printf("  slot=%-3u type=%-18s size=%-4u blob=%zu bytes:", fv->slot, fv->short_name, fv->size,
             fv->blob_len);
...
static void dump_scan(const igb *f, const char *want, const char *slots_csv) {
  ...
  for (int i = 0; i < f->n_objects; ++i) {
    ...
    printf("object[%d] %s %s mem=%zu\n", i, o->type_name, o->is_mem ? "[mem]" : "", o->mem_size);
...
static void dump_anims(const igb *f, int want_idx, int want_frame) {
  ...
  for (int i = 0; i < f->n_objects; ++i) {
    const igb_object *o = &f->objects[i];
    if (o->is_mem || !o->type_name || strcmp(o->type_name, "igEnbayaAnimationSource") != 0) {
      continue;
    }
    ...
    printf("anim[%d] source=%d blob=%d bytes=%zu tracks=%d duration=%.3fs fps=%.1f frames=%d\n", i,
           i, fv->i32, mo->mem_size, a.track_count, a.duration, 1.0f / a.interval, a.frame_count);
    if (want_frame < 0) {
      igb_enbaya_free(&a);
      continue;
    }
    ...
    for (int t = 0; t < a.track_count; ++t) {
      ...
      printf("  t%-3d quat(% .4f,% .4f,% .4f,% .4f) pos(% .3f,% .3f,% .3f)\n", t, p->quat[0],
             p->quat[1], p->quat[2], p->quat[3], p->pos[0], p->pos[1], p->pos[2]);
    }
    igb_enbaya_free(&a);
    if (want_idx >= 0) {
      break;
...
int main(int argc, char **argv) {
  ...
  printf("version=%d flags(info=%d ext=%d shared=%d mpool=%d) objects=%d info_index=%d\n",
         f.version, f.has_info, f.has_external, f.shared_entries, f.has_memory_pool_names,
         f.n_objects, f.info_list_index);
  ...
  for (int i = 0; i < n; ++i) {
    printf("  [%d] %dx%d pfmt=%d size=%d bpr=%d comp=%d datalen=%zu name=%s\n", i, imgs[i].width,
           imgs[i].height, imgs[i].pixel_format, imgs[i].image_size, imgs[i].bytes_per_row,
           imgs[i].compressed, imgs[i].data_len, imgs[i].name ? imgs[i].name : "-");
  }
  ...
  if (argc >= 3) {
    ...
    if (idx >= 0 && idx < n) {
      ...
      if (rgba) {
        printf("rgba %dx%d len=%d (expected %d)\n", imgs[idx].width, imgs[idx].height, len,
               imgs[idx].width * imgs[idx].height * 4);
        if (argc >= 4) {
          FILE *of = igb_file_open(argv[3], "wb");
          if (of) {
